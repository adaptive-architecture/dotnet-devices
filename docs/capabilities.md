...
| Document formats | A printer command language (ZPL, EPL, CPCL, ESC/POS) is mapped to a `document-format` the IPP peer accepts without converting the job. A PDF prints four ways: passed to a local CUPS queue, passed to a `cups://` queue, passed to an IPP printer that reads it, or rendered to PWG Raster for one that does not. `PrinterDevice.Accepts` says whether one channel reads a content type, before a job is sent. |
| Raster | `PwgRasterWriter` writes PWG Raster (PWG 5102.4) in `srgb_8` or `sgray_8`, with the duplex back-side transforms the printer asked for. Public, so an application with its own rasterizer can use it. |
